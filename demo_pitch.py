def get_onsets(filename):
    ...
    timestamps = []
    npsmoothed = np.array(confidences)  # convert your 1-D array to a numpy array if it's not, otherwise omit this line
    peak_indices = signal.find_peaks(npsmoothed, height=tolerance)[0]
    for x in peak_indices:
        timestamps.append(
            float("%02d.%02d" % (times[x] / float(samplerate), 100 * ((times[x] / float(samplerate)) % 1))))
    print(timestamps)
